...
git config --global user.email "58266650+SuperSaiyanJcy@users.noreply.github.com"
git config --global credential.helper store
git config --global --unset credential.helper  # ssh基本用不到
git config --global credential.helper manager  # Windows
git config --global credential.helper osxkeychain  # macos
git config --global credential.helper manager-core  # linux
git config --global --list                                  
git config --global init.defaultBranch main     # 设置默认分支为main
git config --global --get init.defaultBranch   
# 提交时转换为 LF，检出时不转换（保持 LF）
git config --global core.autocrlf input          
```
`git config --global core.autocrlf` [模式] 有三个可选值：`true`、`input` 和 `false`。

1. `core.autocrlf = true` (Windows 标准模式)
这是 **Windows** 安装 Git 时的默认推荐设置。
- **检出时 (Checkout)：** Git 会把仓库里的 `LF` 自动转换成 `CRLF` 放到你的工作目录。（为了让你在 Windows 记事本里打开不乱码）。
- **提交时 (Commit)：** Git 会把你修改过的 `CRLF` 自动变回 `LF` 存入仓库。（保持仓库纯净）。
- 
适用人群：
- 纯 Windows 开发。
- 使用这一招，保证了“只有你自己本地是 CRLF，仓库里和同事看到的永远是 LF”。

2. `core.autocrlf = input` (**Mac/Linux 标准模式**)
这是 **Mac** 和 **Linux** 的推荐设置，也是 WSL 用户的首选。
- **检出时 (Checkout)：不做任何转换**。仓库里是啥（通常是 `LF`），拿出来就是啥。
- **提交时 (Commit)：** 防止意外。如果你不小心引入了 `CRLF`，Git 会强制把它转成 `LF` 再存入仓库。

适用人群：
- Mac / Linux 用户。
- WSL 用户（这点很重要，见下文建议）。
- 它的逻辑是：“我本地本来就是用 LF 的，拿出来不用变；但我怕我不小心搞出 CRLF，所以提交时帮我检查一下。”

3. `core.autocrlf = false` (由于某种原因，我不管)
- 检出时：不转换。
- 提交时：不转换。
...
git commit key.c -m "Update key.c only" # 提交一个指定的文件
